...
def set_MQTT_broker(MQTT_client_name, MQTT_broker_name, MQTT_Port, MQTT_qos, MQTT_last_will, wait_secs):
    print('Connecting with MQTT: {0}'.format(MQTT_broker_name))
    # mqttc = mqtt.Client(MQTT_client_name)
    mqttc = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1, MQTT_client_name)
